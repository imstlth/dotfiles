...
impl<'train> Net<'train> {
    ...
    fn new(
        ...
        for n_couche in 0..config.len() - 1 {
            ...
            biais.push(Array::from_shape_simple_fn(config[n_couche + 1], || {
    ...
    fn run(&mut self, inputs: ArrayView<f64, Ix1>) -> Layer {
        ...
        for n_layer in 0..self.biais.len() {
            ...
            self.sums[n_layer] =
                self.weights[n_layer].dot(&self.activations[n_layer]) + &self.biais[n_layer];
    ...
    fn gradient(
        &mut self,
        input: ArrayView<f64, Ix1>,
        output: ArrayView<f64, Ix1>,
    ) -> (Vec<Array<f64, Ix1>>, Vec<Array<f64, Ix2>>) {
    ...
    fn update(
        &mut self,
        nabla_biais: Vec<Array<f64, Ix1>>,
        nabla_weights: Vec<Array<f64, Ix2>>,
        coeff: f64,
    ) {
    ...
    fn learn(&mut self, step: f64, mini_batch_size: usize, epochs: usize, printing: bool) {
        ...
        for n_couche in 0..self.biais.len() {
            ...
            sum_nabla_weights_default
                .push(Array::<f64, Ix2>::zeros(self.weights[n_couche].raw_dim()));
        }
        ...
        for epoch in 0..epochs {
            ...
            let mut training_input_chunks = self
                .training_input
                .axis_chunks_iter(Axis(0), mini_batch_size);
            let mut training_output_chunks = self
                .training_output
                .axis_chunks_iter(Axis(0), mini_batch_size);
            // Le nombre total de mini_batch
            let n_mini_batch =
                (self.training_input.dim().0 as f64 / mini_batch_size as f64).ceil() as usize;
            ...
            for _n_batch in 0..n_mini_batch {
                ...
                for index in 0..input_batch.dim().0 {
                    ...
                    let (nabla_biais, nabla_weights) =
                        self.gradient(slide_input.view(), output_batch.slice(s![index, ..]));
    ...
    fn graph(
        ...
        let drawing_area = BitMapBackend::new(filename, (width, height)).into_drawing_area();
        ...
        // ).unwrap();
        my_chart
            .draw_series(LineSeries::new((0..).zip(guess), &BLACK))
            .unwrap();
    }
    ...
    fn note(&mut self) -> i32 {
        ...
        for n_data in 0..60_000 {
            let network_output = self
                .run(self.training_input.slice(s![n_data, ..]))
                .mapv(f64::round);
...
fn slide_arr<T: Clone>(
    ...
    let arr2dview = arr.into_shape(shape).unwrap();
...
fn mnist_decode_images(filename: &str) -> Array<f64, Ix2> {
    ...
    Array::<f64, Ix2>::from_shape_simple_fn((60_000, 784), || {
        bytes.next().unwrap().unwrap() as f64 / 255.
    })
...
fn encode_weights_biais(dossier: &str, network: &Net) {
    ...
    network
        .biais
        .iter()
        .map(|couche| couche.for_each(|biais| push_w_space(&mut file_content, biais)))
...
fn decode_weights_biais(dossier: &str, network: &mut Net) {
    ...
    for biais_layer in &mut network.biais {
        ...
        let file_biais = infos.by_ref().take(biais_layer.dim());
        // On change la config du réseau actuel
        *biais_layer = file_biais
            .map(
    ...
    for weights_layer in &mut network.weights {
        ...
        let dim_product = weights_layer.raw_dim().as_array_view_mut().product();
        // Encore une fois, si on ne fait pas by_ref(), ça move infos
        let file_weights_flat = infos.by_ref().take(dim_product).map(|n| n.parse().unwrap());
...
fn interactive(network: &mut Net) {
    loop {
        ...
        // Si on rentre un nombre, on fait avec le nombre de la base de données
            ...
            image_input = network
                .training_input
                .slice(s![input.parse::<usize>().unwrap(), ..])
                .to_owned();
        // Sinon, c'est que l'entrée est un chemin et on ouvre l'image
            ...
            image_input = Array::from_iter(img.pixels().map(|p| 1. - (p.2[0] as f64 / 255.)));
        }
        println!();
        ...
        let mut number = -1;
        let mut results: Vec<(i32, f64)> = network
            .run(image_input.view())
            .iter()
            .map(|v| {
                number += 1;
                (number, *v)
    ...
    F: Fn(&String) -> bool,
{
    ...
    let arg_pos = args.iter().position(predicate);
...
fn parse_args() -> (
    ...
    // Sinon, on extrait les informations des arguments de la ligne de commande
        ...
        let encode_after =
            args.contains(&String::from("-E")) || args.contains(&String::from("--encode-after"));
        // S'il faut utiliser les biais et les poids fournis par un fichier
        let decode_instead =
            args.contains(&String::from("-D")) || args.contains(&String::from("--decode-instead"));
        ...
        // CONFIG
        let config: Vec<usize> = match args.first() {
            // OK, là je me suis fait plaisir
...
fn main() {
    ...
    // Sinon on les génère
        ...
        println!("SCORE = {cost}");
        println!(
            "Temps pris pour calculer le cout de 100% du dataset = {} secondes",
            diff.as_secs()
        );
        let start = time::Instant::now();
        ...
        println!("Fait!");
        println!(
            "Temps pris pour l'entrainement du réseau = {} minutes.\n",
