...
-- C'est un peu chiant d'avoir tout ce que tu sélectionne dans copyq
...
require("lazy").setup({
  spec = {
    ...
    { 'xiyaowong/transparent.nvim', version = "*" },
  },
...
-- Les keymaps pour changer de lignes
-- en considérant qu'une ligne trop longue compte pour plusieurs
...
cmp.setup{
  enabled = function()
    ...
    -- Vérifie si le texte avant le curseur ressemble à un chemin
    return (before:match("([~%.%w_/%\\%-]+)$") and before:match("[/%\\%.~]")) or not disabled
  end,
...
-- YA DES PUTAINS DE NOUVEAUX MESSAGES À CHAQUE CARACTÈRE ÉCRIT PUTAIN DE MERDE
-- SI TOUT CE CODE EST EN COMMENTAIRE, YA LE PUTAIN DE PYLINT QUI S'AFFICHE À TOUTES LES FRAPPES
-- SI JE LE GARDE, YA AUSSI DES MESSAGES MAIS ENCORE PLUS GROS ET S'IL Y A TROP D'ERREUR, NVIM CRASH
-- ÇA PUE
